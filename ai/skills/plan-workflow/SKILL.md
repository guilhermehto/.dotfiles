...
Conventions for the local implementation-plan workspace at `<repo-root>/.scriptorum/`. The `/plan`, `/plan-list`, `/work` commands, the `magos-iterator` and `magos-velox` primary agents, and the `magos-artisan` subagent must follow these rules.
...
`/work` is the entry point for the orchestration agents (`magos-velox` for light tasks, `magos-iterator` for deep tasks). It is a thin **router**; it does not do the work itself. See `commands/work.md` for the full behaviour. The relevant facts here:
...
- `/work <task>` → recommend switching into `magos-velox` with `<task>`.
- `/work --deep <task>` or `/work -d <task>` → recommend switching into `magos-iterator` with `<task>`.

Because opencode commands run inside the current agent and cannot directly switch primary agents, `/work` prints the recommended `@magos-velox <args>` / `@magos-iterator <args>` invocation and stops. The user runs it.
