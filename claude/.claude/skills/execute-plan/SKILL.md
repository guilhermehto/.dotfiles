---
name: execute-plan
description: Execute a local .scriptorum/ plan phase-by-phase — dispatch enginseer per step, run parallel phases concurrently, tick progress, and close with a diff review. Use when the user says /execute-plan, asks to run or implement an existing .scriptorum/ plan, or says "go"/"run the plan" after a plan has been written.
argument-hint: "[slug]"
---
