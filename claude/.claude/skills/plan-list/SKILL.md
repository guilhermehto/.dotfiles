---
name: plan-list
description: List local plans in .scriptorum/ with status, last-updated date, and goal. Use when the user says /plan-list, asks what plans exist or are in progress, or wants to see tracked .scriptorum/ plan status. Read-only.
...
5. **Apply filter.** If `$ARGUMENTS` is provided, keep only files whose derived `<slug>` contains the filter as a substring, case-insensitive.
